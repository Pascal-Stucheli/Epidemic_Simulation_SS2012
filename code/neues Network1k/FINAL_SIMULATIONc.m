...
%parfor b=1:2
    ...
    generate_output(output_array, tot_pop,cities,root,edges);
...
function generate_output(output_array, tot_pop, b, cities, root,edges)
...
for i = 1:length(output_array(1,:))
    ...
    if totinfected(i) >= 0.3 && z == 0
        min_30_percent = i;
        z = 1;
    end
end
ratio_name='ratio000.txt';
ratio_name(9-length(b):8)=int2str(b);
...
percentage_name(14-length(b):13)=int2str(b);
...
output_degree(2) = min_30_percent;
degree_corr_name='degree_corr000.txt';
degree_corr_name(15-length(b):14)=int2str(b);
